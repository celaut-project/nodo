...
from src.virtualizers.microvm.cgroups import CGROUPS_BASE_DIR
from src.virtualizers.microvm.process import pid_alive
from src.virtualizers.microvm.runtime_state import load_runtime_state, recorded_process_name
...
def _process_snapshot(pid: int, process_name: str = "") -> Dict[str, Optional[Any]]:
    if pid <= 0:
        return {"alive": False, "uptime_s": None, "mem_rss_bytes": None}
    if process_name and not pid_alive(pid=pid, process_name=process_name):
        return {"alive": False, "uptime_s": None, "mem_rss_bytes": None}
...
def get_vm_runtime_snapshot(
    ...
    proc = _process_snapshot(pid, process_name=recorded_process_name(runtime_state))
