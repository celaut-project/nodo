...
def grpc_component() -> Tuple[Tuple[str, ...], str, bytes]:
    ...
    prose = (
        ...
        "ResolveNetwork takes a Service.Network and returns the peer addresses this "
        "node believes answer it, resolved by whatever mechanism its tag calls for -- "
        "DNS, a chain crawl, a published endpoint list -- so a caller need not know "
        "which one in advance; to another node the reply is only a belief, verified "
        "like any other address, not a grant.\n"
        "\n"
