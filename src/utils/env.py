...
env_manager.get_env("TRUST_METADATA_ARCHITECTURE", False)  # Only for debug purposes.
...
env_manager.get_env("GATEWAY_PORT", get_free_port(open_port=True))
...
env_manager.get_env("TOTAL_REFILLED_DEPOSIT", pow(10, 64))
env_manager.get_env("MIN_DEPOSIT_PEER", pow(10, 32))
...
env_manager.get_env("BUILD_COST", pow(10, 1))  # Constant if container build is needed.
...
env_manager.get_env("SHORT_INTERVAL_COUNT", 100)
