...
| Every tag must pass | A non-empty whitelist has to cover each tag of each declared network. A network is not one destination, it is as many as it names: `resolve_network` resolves every tag and accumulates their peers (#390), and the firewall reads them one by one too. A tag nobody vetted is a destination nobody vetted. |
