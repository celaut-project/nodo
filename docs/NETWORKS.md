...
the node could not read — the same hole a `pow:` guest's grant is narrowed to avoid,
by not opening a peer's REST slot alongside its P2P one unless the guest's own
declaration explicitly asked for it.
...
| Checked over REST, handed over **both, tagged apart** | A candidate is *verified* at its `restApiUrl` — only the REST API can answer "does your main chain contain B". Each `Instance` then carries **two slots**, each its own `Api.Slot.protocol_stack` tag: the **P2P** one (`pow_networks.P2P_SLOT_TAG`) a chain guest actually wants, and the **REST** one (`pow_networks.REST_SLOT_TAG`) this node just verified it at — never the requester's own `protocol_stack` echoed onto either, which describes what the *asker* wants, identically for every candidate, not what a specific peer's port speaks. Building both lets this node's own peer discovery (`network_discovery.ask_peer`) read a suggestion's REST port back off another node's answer instead of guessing at one (see the row below). |
| What a guest actually gets | **Narrowed to the slot it asked for.** A bare `pow:ergo` tag with no `protocol_stack` — the overwhelming majority of real declarations — keeps only the P2P slot in its own `NetworkResolution` and its firewall grant (`pow_networks.narrow_instances_for_local_grant`, called from both `resolve_network_for_peer` and `rootfs.build_network_resolution`): emitting the REST address to a guest that only asked for a chain peer would be granting egress that could not be used for what it was granted for — both useless and open, the one thing a firewall rule must not be. A guest whose own declared `protocol_stack` explicitly names the REST tag gets both. A remote node asking as a peer over `Gateway.ResolveNetwork` is unaffected either way: nothing it does opens a firewall on the strength of the answer, so it always sees the full picture. |
| Where the P2P port comes from | **Observed, not assumed, wherever anything observed it.** The crawl keeps each peer's `/peers/connected` `address` (`p2pAddress`), so a peer on 9031 is reached on 9031 — of 58 peers on one live mainnet node, five were not on the conventional port. A candidate whose P2P endpoint nobody ever saw (`NODE_URL`, `default_instances`, a peer's `ResolveNetwork` answer, or a crawl entry written before this field existed) reuses the REST **host** with Ergo mainnet's conventional P2P port (`src/manager/ergo.py::MAINNET_P2P_PORT`, 9030 — not a setting, since nothing here can tell which peers run elsewhere to bias a guess towards), and every such assumption is logged. Reusing the host is not an assumption of the same kind — that is where the node that answered `/info` lives; only the port is a guess. Ergo's `/info` does not report the node's own P2P address, so there is no way to ask a candidate for it directly. The REST port, by contrast, is never guessed for a candidate this node itself resolved: it is exactly the `restApiUrl` just verified. A peer-*suggested* REST port (`network_discovery.ask_peer`) is read off that peer's own tagged slot when it has one, and Ergo mainnet's conventional REST port (`src/manager/ergo.py::MAINNET_REST_PORT`, 9053) only when it does not. |
...
| Firewall | `configure_guest_firewall_policy` opens exactly the uris of each peer instance it is handed — which, for a guest, is already narrowed to the slots it asked for. Nothing downstream re-derives a port: the resolver put a concrete one in every uri, which is the point of doing it there. |
| Implementation | `src/manager/pow_networks.py`, dispatched from `resolve_network()`; the P2P address is carried through the crawl by `src/manager/ergo.py`; the guest-facing narrowing is called from `src/manager/networks.py::resolve_network_for_peer` (deferred) and `src/virtualizers/microvm/rootfs.py::build_network_resolution` (eager, at launch) |
