...
- **Donation Models**: A node donates a share of its **earnings** — 2% by default — to the wallets listed in `ledgers.ergo.payments.DONATION_WALLETS`. The figure is shown and can be changed **the first time you run `nodo`**, right after this document; setting `DONATION_PERCENTAGE` to `0` opts out, and one edit of `config.yaml` changes it at any time. Nothing is enforced: what a donation buys is a bounded bonus in *other* nodes' peer selection, computed from the chain. See `docs/DONATIONS.md`.
