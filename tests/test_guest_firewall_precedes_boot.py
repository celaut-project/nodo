...
`_configure_guest_firewall_policy` (block_all + the allow-list) used to run after
`_wait_guest_network_ready` -- seconds after `subprocess.Popen`, and well after
`_create_tap` enslaved the guest's tap to the bridge and made it forwarding-capable.
Nothing that call needs (vm_ip, network_resolution, the gateway address/port) comes
from the guest itself; all of it is resolved earlier in `execute()` from the
manifest and config. So the gap between "guest can forward packets" and "nodo's own
allow-list exists" was pure ordering, not a data dependency -- and during it, a
booting guest's traffic answered only to the host's own default FORWARD policy
(unrestricted on a plain install), not to nodo's.
...
matters: `_configure_guest_firewall_policy` is called after the tap exists and
before the hypervisor process starts, so a future edit that moves it back after
`Popen` fails a fast, dependency-free test instead of only showing up as a live
firewall gap.
"""
import inspect
import unittest
...
class FirewallBeforeBootOrderingTests(unittest.TestCase):
    def _assert_firewall_precedes_boot(self, module, firewall_call, popen_needle="subprocess.Popen("):
        ...
        tap_at = source.index("_create_tap(")
        ...
        self.assertLess(
            ...
            "_configure_guest_firewall_policy must run before the hypervisor "
            "process starts -- moving it after Popen (or after "
            "_wait_guest_network_ready) reopens the window in which a booting "
    ...
    def test_ch_backend_commits_the_policy_before_the_vm_boots(self):
        self._assert_firewall_precedes_boot(
            ch_execute, "_configure_guest_firewall_policy(\n"
        )

    def test_qemu_backend_commits_the_policy_before_the_vm_boots(self):
        self._assert_firewall_precedes_boot(
            qemu_execute, "ch_exec._configure_guest_firewall_policy(\n"
        )
    ...
    def test_the_policy_call_still_precedes_network_readiness_too(self):
        ...
        for module, needle in (
            (ch_execute, "_wait_guest_network_ready("),
            (qemu_execute, "ch_exec._wait_guest_network_ready("),
        ):
            with self.subTest(module=module.__name__):
                source = inspect.getsource(module.execute)
                firewall_at = min(
                    idx for idx in (
                        source.find("_configure_guest_firewall_policy("),
                    ) if idx != -1
                )
                ready_at = source.index(needle)
