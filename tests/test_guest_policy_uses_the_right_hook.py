...
`_configure_guest_firewall_policy` writes three kinds of allow, and they do not
...
a refactor that treats "allow this destination" as one operation.
"""
import unittest
...
class PolicyHookTests(unittest.TestCase):
    def _configure(self, network_resolution=()):
        ...
        with patch.object(ch_execute, "vm_allow_host_connection", side_effect=_host), \
             patch.object(ch_execute, "vm_block_all", side_effect=_block_all), \
             patch.object(ch_execute, "vm_allow_connection_to_instance", side_effect=_to_instance), \
             patch.object(ch_execute, "vm_allow_all_egress", side_effect=_all_egress), \
             patch.object(ch_execute.env_manager, "get_gateway_port", return_value=GATEWAY_PORT):
    ...
    def test_a_failure_on_the_host_hook_aborts_the_launch(self):
        with patch.object(ch_execute, "vm_block_all", return_value=True), \
             patch.object(ch_execute, "vm_allow_host_connection", return_value=False), \
             patch.object(ch_execute.env_manager, "get_gateway_port", return_value=GATEWAY_PORT):
            with self.assertRaises(ch_execute.CHExecuteError) as ctx:
                ch_execute._configure_guest_firewall_policy(
                    vmachine_id=VM, vm_ip=VM_IP, network_resolution=[]
                )
